...
public class WhoCommand extends MultiverseCommand {
    ...
    public WhoCommand(final MultiverseCore plugin) {
        super(plugin);
        setName("Who?");
    ...
    private static String buildPlayerString(final MultiverseWorld world, final Player viewer, final Collection<Player> visiblePlayers) {
        // Retrieve the players in this world
        final List<Player> players = world.getCBWorld().getPlayers();
        ...
        final String bString = playerBuilder.toString();
    ...
    public void runCommand(final CommandSender sender, final List<String> args) {
        ...
        if (args.size()  == 1) {
            if (args.get(0).equalsIgnoreCase("--all") || args.get(0).equalsIgnoreCase("-a")) {
                ...
                // single world mode
                final MultiverseWorld world = worldManager.getMVWorld(args.get(0));
                if (world == null) {
                    sender.sendMessage(ChatColor.RED + "That world does not exist.");
                    return;
                }

                if (!plugin.getMVPerms().canEnterWorld(p, world)) {
        ...
        for (final MultiverseWorld world : worldManager.getMVWorlds()) {
            if (plugin.getMVPerms().canEnterWorld(p, world)) { // only show world if the player can access it
